...
function AIChatboxPage() {
  ...
  useEffect(() => {
    async function loadInitial() {
      ...
      if (
        Array.isArray(data.quick_questions) &&
        data.quick_questions.length
      ) {
    ...
    <main className="min-h-screen bg-[#f5f5f7] px-8 py-8">
      <div className="max-w-[1440px] mx-auto flex flex-col gap-6">
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          {/* ===== Chat Area ===== */}
          <section aria-label="Chat area" className="xl:col-span-2 flex flex-col">
            <div className="bg-white rounded-t-3xl flex flex-col h-[520px] overflow-hidden">

              {/* ===== HEADER ICON (ANTI GEPENG) ===== */}
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4 bg-[#efefef] space-y-4">
                ...
                {isLoading && <ChatMessage role="assistant" isTyping={true} />}
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                {suggestions.map((s) => (
                  <button
                    ...
                    className="px-4 py-2 rounded-[10px] bg-[#1C2534] text-xs text-white hover:bg-[#344054] transition-colors duration-150"
                  >
                    {s}
                  </button>
                ))}
